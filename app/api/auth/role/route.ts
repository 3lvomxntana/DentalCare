...
export async function GET(req: Request) {
  try {
    ...
    try {
      const doc = await api(`/databases/${DB}/collections/${usersCollection}/documents/${userId}`)
      const role = doc?.data?.role || doc?.role || null
      return NextResponse.json({ ok: true, role, profile: doc })
    } catch (err: any) {
      return NextResponse.json({ ok: false, message: err?.message || String(err) }, { status: 500 })
    }
  } catch (err: any) {
    console.error('/api/auth/role GET failed', err)
    return NextResponse.json({ ok: false, message: err?.message || String(err) }, { status: 500 })
  }
}
